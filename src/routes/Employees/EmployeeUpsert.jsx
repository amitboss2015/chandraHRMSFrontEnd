...
// Dynamic Allowances inline component for Edit Employee Salary & Allowances
function DynamicAllowancesInline({ employeeId }) {
        ...
        <>
          <div className="space-y-2">
            {assigned.length === 0 ? (
              ...
              assigned.map((a) => {
                ...
                return (
                  <div key={a.id} className="flex justify-between items-center py-2 px-3 bg-amber-50 rounded-lg border border-amber-100">
                    <div>
                    ...
                    <button type="button" className="text-red-600 hover:text-red-700 text-sm px-2 py-1" onClick={() => remove(a.allowanceTypeId)}>
                      Remove
                    </button>
                  </div>
                );
          ...
          {adding ? (
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <button type="button" className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm" onClick={assign} disabled={!selectedTypeId}>
                Assign
              </button>
              <button type="button" className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 text-sm" onClick={() => { setAdding(false); setSelectedTypeId(""); }}>
                Cancel
              </button>
            </div>
          ) : (
        ...
        <div className="flex gap-6">
          ...
          <form onSubmit={onSubmit} className="flex-1 space-y-6">
            ...
            >
              ...
              {/* Dynamic Allowances (attendance-based, e.g. fare per present day) */}
              {isEdit && employeeId && (
                <div className="mt-6 pt-4 border-t">
                  <h4 className="text-sm font-medium text-slate-700 mb-2">Dynamic Allowances (Attendance-based)</h4>
                  <p className="text-xs text-slate-500 mb-3">
                    e.g. fare per present day – calculated from attendance. Create types in Settings → Allowance Types.
                  </p>
                  <DynamicAllowancesInline employeeId={employeeId} />
                </div>
              )}
            </Section>
